...
# スクリーンショットし、画像寸法と復号後のRGBAを完全比較する。
...
# 既定は全ピクセル完全一致。実装上の差異はregistryで理由と領域を限定する。
...
# シナリオと例外領域はcomponent coverage registryを唯一の情報源とする。
PARITY_SCENARIOS = ComponentCoverage.parity_scenarios.freeze

# 撮影前の両ページを同じ表示条件へ揃える
def parity_prepare(page, dark: false, min_height: 1)
  # animate-pulse 等のアニメーションを両側で同じように止めてからbody要素を撮る
...
RSpec.describe "visual parity", :parity, type: :system do
  ...
  PARITY_SCENARIOS.each do |scenario|
    ...
    PARITY_MODES.each do |mode|
      dark = mode == :dark
      regions = if dark
                  scenario.fetch("dark_allowed_regions", scenario.fetch("allowed_regions", []))
                else
                  scenario.fetch("allowed_regions", [])
                end
      it "#{ours_path} が upstream(#{demo_id}) と一致する(#{mode})" do
        ...
        parity_prepare(page, dark: dark, min_height: min_height)
        if demo_id == "select/default"
          find("[data-slot='select-trigger']").click
          find("[data-slot='select-content']")
        end
        upstream_bg = parity_capture(page, upstream_png)
        ...
        ours_bg = parity_capture(page, ours_png)
        ...
        out, status = Open3.capture2e("node", PARITY_COMPARE, ours_png, upstream_png, diff_png, report_json, regions.to_json)
        report = JSON.parse(File.read(report_json))
        expect(report.fetch("pass")).to be(true), <<~MSG
          #{ours_path} のupstreamとのRGBAまたは画像寸法が一致しません(#{mode}):
          全差分 #{report['differing']}px / 許容領域内 #{report['allowedDiffering']}px / 領域外 #{report['unallowedDiffering']}px

          #{"#{out}\n" unless status.success?}
          artifacts: #{dir}
            ours.png / upstream.png / diff.png(赤=未許容、橙=許容領域内)
