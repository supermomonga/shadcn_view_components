...
- 対象読者: コントリビューター全員
- 関連ドキュメント: [00-overview](00-overview.md) / [05-stimulus-hotwire](05-stimulus-hotwire.md) / [09-ci-drift-detection](09-ci-drift-detection.md)
...
| 作業 | 内容 | ドキュメント |
|---|---|---|
| gem雛形 | gemspec・エンジン・ディレクトリ構成（[01](01-architecture.md) §2） | 01 |
| 同期 | `rake shadcn:sync` / manifest（[02](02-upstream-sync.md) §5.1） | 02 |
| 抽出器 | Babel AST解析・契約JSON生成（[03](03-extraction-codegen.md)）: cva/cn/data-slot/事前解決 | 03 |
| 生成器 | Ruby・CSS出力・正規化（[03](03-extraction-codegen.md) §6-7） | 03 |
| BaseComponent | 属性マージ・クラス解決・tag: 差し替え（[04](04-component-conventions.md) §5-6） | 04 |
| **Button** | パイプライン実証として実装。適合試験・プレビュー込み | 04/07 |
| CI | ci.yml（determinism含む）+ upstream-drift.yml（[09](09-ci-drift-detection.md)） | 09 |
| Sorbet | tapioca・strict全面・CI統合（[08](08-sorbet.md)） | 08 |
| テスト基盤 | spec/dummy・Lookbook・Cuprite・conformanceマッチャ（[07](07-testing.md)） | 07 |
| インストーラ | `shadcn_view_components:install`（[06](06-theming-tailwind.md) §5） | 06 |
...
ここで「新規コンポーネント追加の最短経路」（[04](04-component-conventions.md) §7チェックリスト）を安定させる。
...
- [ ] 全コンポーネントが[04](04-component-conventions.md) §7チェックリストを満たす
...
| コンポーネント | 実装基盤（[05](05-stimulus-hotwire.md) §3） |
|---|---|
...
- [ ] JS無効時フォールバック分類（Graceful/Readable）が全コンポーネントで判定済み（[05](05-stimulus-hotwire.md) §5）
- [ ] キーボード操作チェックリスト（[05](05-stimulus-hotwire.md) §4）のシステムスペック緑
...
- [ ] Turbo Frame/キャッシュ併用の統合スペック（[05](05-stimulus-hotwire.md) §6）緑
...
| 決定ログ（[00-overview](00-overview.md) §5）の更新 | 技術的デフォルトに変更加えるたび |
...
| リスク | 影響フェーズ | 対策 | 参考 |
|---|---|---|---|
| upstreamのTSX新構文で抽出不能 | 全フェーズ | fail-fast + 週次検知 + 抽出器の局所修正 | [03](03-extraction-codegen.md) §8 |
| ネイティブ要素でARIA/操作品質がAPGに届かない | 3, 4 | システムスペックのゲート。不足時はコントローラで補完（方針 #5の範囲内） | [05](05-stimulus-hotwire.md) |
| Tailwindビルド検証で未対応ユーティリティ発覚 | 1以降 | `tailwind-build` ジョブでの検知。upstream要件をREADME明記 | [06](06-theming-tailwind.md) §6 |
| chart/data-tableの再実装コスト超過 | 4 | 個別評価にてスコープ外も選択肢とする | 本ドキュメント §6 |
| 週次PRのレビュー滞留 | 1以降 | PRは常に1件まで。滞留はmanifestで可視 | [09](09-ci-drift-detection.md) §4 |
