...
- 関連ドキュメント: [01-architecture](01-architecture.md) / [03-extraction-codegen](03-extraction-codegen.md) / [05-stimulus-hotwire](05-stimulus-hotwire.md) / [08-sorbet](08-sorbet.md)
...
- 規約自体が契約（[03](03-extraction-codegen.md)）と並ぶ第二の契約として、ドキュメントと実装の乖離を防ぐ
...
| Stimulusコントローラ | `shadcn/<component>_controller.js`、data-controller名 `shadcn--<component>` | [05](05-stimulus-hotwire.md) §2 |
| 契約モジュール | `ShadcnViewComponents::Contracts::<PascalCase>`（生成物） | [03](03-extraction-codegen.md) §6.1 |
...
- **erbsorbetは使わない**。ERBはロジックを持たない程度に薄く保ち、型検査対象はRubyコードに集中させる（[08-sorbet](08-sorbet.md) §4）
...
| `forceMount` 等の制御props | それぞれ個別設計 | [05](05-stimulus-hotwire.md) でコンポーネント毎に定義 |
...
静的属性（`static_attributes`）についても契約との一致を検証する（[07-testing](07-testing.md) §3）。
...
- [ ] システムスペック（インタラクティブの場合）があるか（[07-testing](07-testing.md) §5）
- [ ] Lookbookプレビューがあるか（[07-testing](07-testing.md) §7）
- [ ] `srb tc` / `rubocop` が緑か
