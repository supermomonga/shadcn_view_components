...
- 関連ドキュメント: [01-architecture](architecture.md) / [03-extraction-codegen](extraction-codegen.md) §6.2 / [02-upstream-sync](upstream-sync.md)
...
| 項目 | 方針 | 理由 |
|---|---|---|
| Tailwindバージョン | **v4のみ**（CSS-first設定） | upstreamがv4移行済み。v3の併立サポートは追従コストを倍加させるためノンゴール（[00-overview](overview.md) §2） |
...
抽出器が全アイテムの `cssVars` / `css` フィールドから生成する（生成規則は [03](extraction-codegen.md) §6.2）:
...
- クラスを上書きするカスタマイズ（`class:` 引数でのtailwind-merge統合）も併用可（[04](component-conventions.md) §6）
...
importmap-rails利用時はエンジンが自動pin（[01](architecture.md) §3.2）。手動pinのフォールバック行もREADMEに記載する。
