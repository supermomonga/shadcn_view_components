...
- 関連ドキュメント: [00-overview](overview.md) / [03-extraction-codegen](extraction-codegen.md) / [09-ci-drift-detection](ci-drift-detection.md)
...
URL構造の変更が同期コードの一点に閉じるようにする（[03-extraction-codegen](extraction-codegen.md) §2 の `fetch.ts`）。
...
4. `fetched_at` / `checked_at` は対応するcontent hash / release SHAが変わった場合だけ更新する。
   **契約JSON等の他の生成物にタイムスタンプを入れない**
   （冪等性の要件。詳細は [03-extraction-codegen](extraction-codegen.md) §7）
...
### 6.1 通常（週次自動、詳細は [09-ci-drift-detection](ci-drift-detection.md)）
...
- upstreamコンポーネントにはバージョンがないため、**本gemのバージョンは独自semver** を持つ
  （開発中は0.x、API安定後に1.0。詳細は [09-ci-drift-detection](ci-drift-detection.md) §6）
...
| アイテムの廃止 | 提供コンポーネントが宙に浮く | syncが削除を警告。deprecation期間を設けてgemのマイナーリリースで廃止（[09](ci-drift-detection.md) §6） |
...
| 大量の同時変更（破壊的リニューアル） | 自動PRが巨大化しレビュー困難 | この場合はPRを分割せず「メジャーバージョンアップ」として扱う運用ルールを [09](ci-drift-detection.md) §5 に定義 |
