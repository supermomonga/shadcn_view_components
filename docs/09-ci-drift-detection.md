...
- 関連ドキュメント: [00-overview](00-overview.md) / [02-upstream-sync](02-upstream-sync.md) / [03-extraction-codegen](03-extraction-codegen.md) / [07-testing](07-testing.md)
...
| ジョブ | 内容 | 使用ツール |
|---|---|---|
| `lint-ruby` | `rubocop`（rubocop-sorbet含む。[08-sorbet](08-sorbet.md) §7） | mise |
...
| `tailwind-build` | ビルド済みgemのEngine統合と全クラス抽出元が実際にTailwindビルドを通るか（[06](06-theming-tailwind.md) §6） | `tailwindcss-rails` 4.3系 / lock解決版 + node |
...
[3] rake shadcn:update                 # sync + generate（[02](02-upstream-sync.md) §5.4）
...
| extract失敗（AST解析不能） | 同上。この場合抽出器の対応が必要（[03](03-extraction-codegen.md) §8） |
...
- メジャー対応では、現行スナップショットを維持したリリースブランチ（`maintenance/v0` 等）と、
  ...
  固定はgit履歴から常に再現可能**（[02](02-upstream-sync.md) §7）であるため、分離コストは低い
...
- syncがアイテム削除を警告した場合（[02](02-upstream-sync.md) §5.1 [2]）:
...
- upstreamでのリネームは「削除+新規」で検知されるため、同名判定・エイリアス提案は人間が判断して
  命名表（[04](04-component-conventions.md) §2）に記録する
...
- `vendor/` 取得はHTTPS固定。生成前のZod検証がサプライチェーンのゲート（[03](03-extraction-codegen.md) §4）
