...
function renderExportConstants(exportData: Export): string[] {
  ...
  if (exportData.classes_slot !== exportData.root_slot) {
...
export function renderContractRuby(contract: Contract, style: string): string {
  ...
  return [
    ...
    `# Source: shadcn/ui ${style} "${contract.name}" (item sha256: ${contract.source.item_sha256})`,
...
export async function emitContractRuby(contract: Contract, rubyDir: string, style: string): Promise<string> {
  const filePath = `${rubyDir}/${contract.name}.rb`
  await atomicWriteFile(filePath, renderContractRuby(contract, style))
  return filePath
}
