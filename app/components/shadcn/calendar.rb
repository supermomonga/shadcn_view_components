  ...
  class Calendar < BaseComponent
    ...
    def call
      content_tag(:div, **html_attributes) do
        safe_join([navigation, month_grid])
      end
    end
    ...
    # 年月キャプションと前後月への移動(month_path が与えられたときのみリンク)
    sig { returns(String) }
    def navigation
      content_tag(:div, class: "relative flex w-full items-center justify-between gap-1") do
        safe_join([
                    nav_link(direction: :previous, label: "前の月") { chevron(:left) },
                    content_tag(:div, class: "font-medium select-none text-sm") do
    ...
    def nav_link(direction:, label:, &block)
      ...
      content_tag(:a,
                  ...
                  class: "inline-flex items-center justify-center rounded-md size-9 p-0 select-none " \
    ...
    sig { returns(String) }
    def month_grid
      content_tag(:table, class: "w-full border-collapse") do
